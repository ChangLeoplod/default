...
//保存cookie
orderCookie.storageSave = function( key, value, type ) {
    if(type!=1){
        var string = $.isPlainObject( value ) ? JSON.stringify(value) : value;
        $.cookie( key, string, {expires:30, path:'/'} );
        return true;
    }else{
        orderCookie.saveSession(key,value);
    }

};
//保存session
orderCookie.saveSession = function(key,value){
    $.ajax({
        type:'POST',
        url:siteUrl+'jieban/ajax.jieban.php',
        data:{action:'savesession',key:key,value:value},
...
//获取session
orderCookie.getSession = function(key){
    $.ajax({
        type:'POST',
        url:siteUrl+'jieban/ajax.jieban.php',
        data:{action:'getsession',key:key},
        async:false,
        success:function(data){
            return data;
        }
...
var jieban={
    ...
    storageSave : function(key,value,type){
         type = type!=1 ? undefined : type;
         orderCookie.storageSave(key,value,type)
    },
    ...
    getSession:function(key){
        return orderCookie.getSession(key);
    },
    ...
    step7BindEvent:function(){
        $("#pg7").find('.btn_confirm').click(function(){
            //var memo = $('#memo').val();
            var memo = memoEditor.getContent();
            if(memo!=''){
                jieban.storageSave('u_memo',memo,1);
    ...
    step8BindEvent:function(){
      ...
      $("#pg8").find('.btn_confirm').unbind('click').click(function(){
    ...
    readSaveJieban:function(){ //读取用户已填写信息
        ...
        var u_memo = jieban.getSession('u_memo');
    ...
    step11:function(){  //新增加的添加标题功能.
        jieban.showPage('pg11');
        jieban.step11BindEvent();
    },
    step11BindEvent:function(){

        //确认选择
        $("#pg11").find('.btn_confirm').click(function(){
            var title = $("#title").val();

            if(title==''){
                layer.msg('请填写你的活动标题', 2,8);
                return false;
            }

            jieban.storageSave('u_title',title);
            var _html="我的活动标题："+title;
            $('#pg11_ans').find('.im-you-txt').html(_html);
            jieban.showPage('pg11_ans');
            jieban.step3();
