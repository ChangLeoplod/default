...
function GetPreNext($id)
{
	...
	$preRow =  $dsql->GetOne("Select aid,articlename,shownum From #@__article where aid<$aid and webid='0' order by aid desc");
	$nextRow = $dsql->GetOne("Select aid,articlename,shownum From #@__article where aid>$aid And webid='0' order by aid asc");

   
	if(is_array($preRow))
	{
		if(mb_strlen($preRow['articlename'],'utf-8')>22)
		{
		$pre_article=mb_substr($preRow['articlename'],0,22,'utf-8').'...';
		}
		else
		  $pre_article=$preRow['articlename'];
	...
	if(is_array($nextRow))
	{
		...
		if(mb_strlen($nextRow['articlename'],'utf-8')>22)
		{
		  $next_article=mb_substr($nextRow['articlename'],0,22,'utf-8').'...';
		}
		else
		  $next_article=$nextRow['articlename'];
