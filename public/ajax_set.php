...
{
	$offset = $num * $pagesize;
	$fields="distinct a.aid,a.webid,a.linename,a.seotitle,a.sellpoint,a.linepic as litpic,a.storeprice,a.lineprice,a.linedate,a.transport,a.lineday,a.startcity,a.overcity,a.shownum,a.satisfyscore,a.bookcount";
	...
	foreach($res AS $row)
	{
		...
		$lprice = empty($row['lineprice']) ? '电询' : '<span class="rmb_1">￥</span>' . $row['lineprice'];
		//$point = empty($row['sellpoint']) ? '' : getSellpoint($row['sellpoint']);
		$html .= '<ul class="l-cl">
                      <li class="cl-img">
                          <a href="' . $url . '"><img src="' . $litpic . '" alt="' . $row['linename'] . '" width="112" height="84" /></a>
                      </li>
                      <li class="cp_pb_ms">
                          <span><a href="' . $url . '">' . $row['linename'] . $extend . '</a></span>' . '
