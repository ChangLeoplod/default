    ...
    function () {
        ...
        window.product_store = Ext.create('Ext.data.Store', {
            ...
            proxy: {
                type: 'ajax',
                api: {
                    read: SITEURL+'order/dz/action/read/',  //读取数据的URL
                    update: SITEURL+'order/dz/action/save/',
                    destroy: SITEURL+'order/dz/action/delete/'
                },
                reader: {
                    type: 'json',   //获取数据的格式
                    root: 'lists',
            ...
            columns: [
                ...
                },
                {
                    ...
                    renderer: function (value, metadata, record) {
                        ...
                        var id = record.get('id');
                        var html = "<a href='javascript:void(0);' class='row-del-btn' title='删除' onclick=\"delS(" + id + ")\"></a>";
                        return html;
                        // return getExpandableImage(value, metadata,record);
                    }
...
function updateField(ele, id, field, value, type) {
    ...
    Ext.Ajax.request({
        url: SITEURL+"order/dz/action/update/",
        method: "POST",
        datatype: "JSON",
...
function view(id)
{
    var url=SITEURL+"order/view/type/dz/id/"+id;
    ST.Util.showBox('查看订单信息',url,600,700,function(){window.product_store.load()});
}
