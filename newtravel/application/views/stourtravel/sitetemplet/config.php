...
<script language="JavaScript">
    ...
    function addEvent()
    {
        ...
        $(".v2").find('.row-mod-btn').unbind('click').click(function(){
            ...
            var url = SITEURL+'filemanager/index/parentkey/templet/itemid/1/issline/1/folder/'+folder+'/';
            ST.Util.addTab(folder+'模板文件查看',url,1);
            ST.Util.closeDialog();

        })
        $(".v2").find('.row-del-btn').unbind('click').click(
            function(){
                ...
                ST.Util.confirmBox("提示","确定删除？",function() {
                    $.ajax({
                        type: 'POST',
                        url: SITEURL + 'sitetemplet/ajax_del',
                        data: {path:path},
                        dataType: 'json',
        ...
        $(".v3").find('input').unbind('click').click(function(){
            ...
            $.ajax({
                type:'POST',
                url:SITEURL+'sitetemplet/ajax_config_save',
                data:{id:id,pageid:pageid,webid:webid},
                dataType:'json',
                success:function(data){
                  if(data.status){
