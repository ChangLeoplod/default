    ...
    <tr>
        ...
        <td valign="top" class="content-rt-td" style="overflow:hidden">
            <div class="crumbs" id="attr_crumbs">
                <label>位置：</label>首页 &gt; 设置中心 &gt; {$channelname}设置 &gt; <span>属性设置</span>
            </div>


            <div></div>
            <div class="w-set-tit bom-arrow" style="float:none;margin-top: 40px">
                {template 'stourtravel/attr/model_attr_top'}
    ...
    function () {
        ...
        window.attr_store = Ext.create('Ext.data.TreeStore', {
            ...
            proxy: {
                ...
                api: {
                    read: SITEURL+'attrid/modelattr/action/read/typeid/{$typeid}',  //读取数据的URL
                    update:  SITEURL+'attrid/modelattr/action/save/typeid/{$typeid}',
                    destroy:  SITEURL+'attrid/modelattr/action/delete/typeid/{$typeid}'
                },
                reader: 'json'
            },
            autoLoad: true,
            ...
            viewConfig: {
                ...
                listeners: {
                    ...
                    drop: function (node, data, overModel, dropPosition, eOpts) {
                        ...
                        Ext.Ajax.request({
                            url: SITEURL+'attrid/modelattr/action/drag/typeid/{$typeid}',
                            params: params,
                            method: 'POST',
                ...
                },
                {
                    ...
                    renderer : function(value, metadata,record) {
                        ...
                        if(id.indexOf('add')!=-1 || pid=='0')
                            return '';
                        return '<a href="javascript:;" onclick="attrSet('+id+','+{$typeid}+')">配置</a>';
                    }
                }
            ],
            plugins: [
...
function addSub(pid) {
    ...
    Ext.Ajax.request({
        method: 'post',
        url: SITEURL+'attrid/modelattr/action/addsub/typeid/{$typeid}',
        params: {pid: pid},
        success: function (response) {
...
function updateField(ele,id,field,value,type)
{
    ...
    Ext.Ajax.request({
        url   :  SITEURL+"attrid/modelattr/action/update/typeid/{$typeid}",
        method  :  "POST",
        datatype  :  "JSON",
