...
class Controller_Car  extends Stourweb_Controller{
	...
	public function action_car()
	{
		...
		else if($action=='read')    //读取列表
		{
			...
			if(empty($kindid))
			{
			   $sql="select  a.id,a.webid,a.aid,a.carname,a.seatnum,a.carkindid,a.carbrandid,a.kindlist,a.attrid,a.iconlist,a.themelist,if(length(ifnull(a.attrid,''))=0,0,1) as order_attrid,if(length(ifnull(a.kindlist,''))=0,0,1) as order_kindlist,if(length(ifnull(a.iconlist,''))=0,0,1) as order_iconlist,if(length(ifnull(a.themelist,''))=0,0,1) as order_themelist
,a.ishidden,c.kindname as brandname,d.kindname as carkindname,IFNULL(b.displayorder,9999) as displayorder,ifnull(e.suitday,0) as suitday from sline_car as a left join sline_allorderlist b on (a.id=b.aid and b.typeid=3) left join sline_car_brand c on (a.carbrandid=c.id and a.webid=0) left join sline_car_kind d on (a.carkindid=d.id) left join (select c.carid,c.id,min(c.suitday) as suitday from(select a.carid,a.id,max(b.day) as suitday
 from sline_car_suit a left join sline_car_suit_price b on a.id=b.suitid  group by a.id) c group by c.carid) e on a.id=e.carid where $w $order limit $start,$limit";
			...
			{
				$sql="select  a.id,a.webid,a.aid,a.carname,a.seatnum,a.carkindid,a.carbrandid,a.kindlist,a.attrid,a.iconlist,a.themelist,if(length(ifnull(a.attrid,''))=0,0,1) as order_attrid,if(length(ifnull(a.kindlist,''))=0,0,1) as order_kindlist,if(length(ifnull(a.iconlist,''))=0,0,1) as order_iconlist,if(length(ifnull(a.themelist,''))=0,0,1) as order_themelist
,a.ishidden,b.isjian,b.isding,b.istejia,ifnull(b.displayorder,9999) as displayorder ,c.kindname as brandname,d.kindname as carkindname,ifnull(e.suitday,0) as suitday from sline_car as a left join sline_kindorderlist as b on (a.id=b.id and b.classid=$kindid and b.typeid=3) left join sline_car_brand c on (a.carbrandid=c.id) left join sline_car_kind d on (a.carkindid=d.id) left join (select c.carid,c.id,min(c.suitday) as suitday from(select a.carid,a.id,max(b.day) as suitday
