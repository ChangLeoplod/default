...
class Controller_Sitetemplet extends Stourweb_Controller{
     ...
     * */
     public function action_index()
     {
         ...
         if(empty($action))
         {
             ...
             $this->display('stourtravel/sitetemplet/list');
         }
         else if($action=='read')
         {
             ...
             if($node=='root')//属性组根
             {
                 $list=ORM::factory($attrtable)->where('pid','=','0')->order_by('id','asc')->get_all();
                 foreach($list as $k=>$v)
                 {
                     $list[$k]['templet'] = '';
             ...
             {
                 $list=ORM::factory($attrtable)->where('pid','=',$node)->order_by('id','asc')->get_all();
                 foreach($list as $k=>$v)
                 {
                     $list[$k]['templet'] = self::getUseTemplet($v['id'],$webid);
                     $list[$k]['leaf']=true;
                 }
                /* $list[]=array(
                     'leaf'=>true,
     ...
     * */
     public function action_config()
     {
         ...
         $templet_list = ORM::factory('site_page_config')->where('pageid','=',$pageid)->and_where('webid','=',$webid)->get_all();

         $this->assign('templet_list',$templet_list);

         $this->assign('pageid',$pageid);
         $this->assign('webid',$webid);
         $this->display('stourtravel/sitetemplet/config');
    ...
    public function action_ajax_del()
    {
        ...
        $model = ORM::factory('site_page_config')->where('path','=',$path)->find();
        Common::rrmdir(BASEPATH.'/templets/sline/uploadtemplets/'.$path);
        if($model->delete())
        {
    ...
    public function action_ajax_config_save()
    {
        ...
        $webid = ARR::get($_POST,'webid');//页面id.
        $id = ARR::get($_POST,'id');//模板id

        $models = ORM::factory('site_page_config')->where('pageid','=',$pageid)->and_where('webid','=',$webid)->find_all();
        foreach($models as $m)
        {
            $m->isuse = 0;
            $m->update();
        ...
        if(!empty($id))
        {
            $model2 = ORM::factory('site_page_config',$id); //置为使用
            $model2->isuse = 1;
            $model2->update();
            if($model2->saved())
            {
    ...
    public function action_ajax_upload_templet()
    {
        ...
        if(move_uploaded_file($filedata['tmp_name'], $storepath))
        {
            ...
            if(self::extractTemplet($storepath,$basefolder))
            {
                ...
                $model = ORM::factory('site_page_config')->where('path','=',$templetname)->find();
                $model->pageid = $pageid;
                $model->webid = $webid;
    ...
    public static function getUseTemplet($pageid,$webid)
    {
        $row = ORM::factory('site_page_config')->where('pageid','=',$pageid)->and_where('webid','=',$webid)->and_where('isuse','=',1)->find()->as_array();
