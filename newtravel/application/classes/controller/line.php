...
class Controller_Line extends Stourweb_Controller{
	...
	public function action_line()
	{
		...
		else if($action=='read')    //读取列表
		{
			...
			if($kindid!=0)
			{
				$sql="select a.id,a.aid,a.linename,a.iconlist,a.lineprice,a.tprice,a.profit,a.startcity,a.lineclassid as classid,if(length(ifnull(a.attrid,''))=0,0,1) as order_attrid,if(length(ifnull(a.kindlist,''))=0,0,1) as order_kindlist,if(length(ifnull(a.iconlist,''))=0,0,1) as order_iconlist,if(length(ifnull(a.themelist,''))=0,0,1) as order_themelist,
a.attrid,a.lineclassid,a.webid,a.kindlist,a.ishidden,a.piclist,a.themelist,a.supplierlist,b.isjian,IFNULL(b.displayorder,9999) as displayorder,b.isding,b.istejia,ifnull(d.suitday,0) as suitday from sline_line as a left join sline_kindorderlist b on (a.id=b.aid and b.typeid=1 and b.classid=$kindid)  left join (select
c.lineid,c.id,min(c.suitday) as suitday from(select a.lineid,a.id,max(ifnull(b.day,0)) as suitday from sline_line_suit a left join sline_line_suit_price b on a.id=b.suitid group by a.id) c group by c.lineid) d on a.id=d.lineid where $w $order limit  $start,$limit";
			...
			{
			    $sql="select a.id,a.aid,a.linename,a.supplierlist,a.iconlist,a.lineprice,a.tprice,a.profit,a.startcity,a.lineclassid as classid,a.attrid,if(length(ifnull(a.attrid,''))=0,0,1) as order_attrid,if(length(ifnull(a.kindlist,''))=0,0,1) as order_kindlist,if(length(ifnull(a.iconlist,''))=0,0,1) as order_iconlist,if(length(ifnull(a.themelist,''))=0,0,1) as order_themelist,a.lineclassid,a.webid,a.kindlist,a.ishidden,a.piclist,a.themelist,b.isjian,IFNULL(b.displayorder,9999) as displayorder,b.isding,b.istejia,ifnull(d.suitday,0) as suitday from sline_line as a left join sline_allorderlist b on (a.id=b.aid and b.typeid=1)   left join (select
c.lineid,c.id,min(c.suitday) as suitday from(select a.lineid,a.id,max(ifnull(b.day,0)) as suitday from sline_line_suit a left join sline_line_suit_price b on a.id=b.suitid group by a.id) c group by c.lineid) d on a.id=d.lineid where $w $order  limit $start,$limit";
			}
    ...
    public function action_edit()
    {
        ...
        if($model->id)
        {
            ...
            $columns=ORM::factory('line_content')->where("webid=0 and isopen=1 and isline=0 and columnname!='linespot' ")->order_by('displayorder','asc')->get_all();
	...
	*/
	public function action_ajax_linesave()
	{
		...
		$data_arr['linetitle']=Arr::get($_POST,'linetitle');
		$data_arr['linesubtitle']=Arr::get($_POST,'linesubtitle');
        ...
        $data_arr['packpic']=Arr::get($_POST,'packpic');
        $data_arr['bigpic']=Arr::get($_POST,'bigpic');
        $data_arr['mobilepic']=Arr::get($_POST,'mobilepic');
        $data_arr['linepic']=Arr::get($_POST,'linepic');
    ...
    public function saveBaoJia($lineid,$suitid,$arr)
    {
        ...
        if($pricerule=='all')
        {
            ...
            while(true)
            {
                ...
                $data_arr['roombalance']=$roombalance;
