...
class Controller_Spot  extends Stourweb_Controller{
	...
	public function action_spot()
	{
		...
		else if($action=='read')    //读取列表
		{
			...
			$w="a.id is not null";
            $w.=empty($keyword)?'':" and (a.spotname like '%{$keyword}%' or a.id like '%{$keyword}%')";
...
a.webid,a.piclist,a.themelist,a.iconlist,a.supplierlist,a.ishidden,b.isjian,ifnull(b.displayorder,9999) as displayorder,b.isding from sline_spot as a left join sline_allorderlist b on (a.id=b.aid and b.typeid=5) where $w $order  limit $start,$limit ";
			...
			{
			    $sql="select a.aid,a.id,a.spotname,a.doorprice,a.area,a.spotpicid,if(length(ifnull(a.attrid,''))=0,0,1) as order_attrid,if(length(ifnull(a.kindlist,''))=0,0,1) as order_kindlist,if(length(ifnull(a.iconlist,''))=0,0,1) as order_iconlist,if(length(ifnull(a.themelist,''))=0,0,1) as order_themelist,
   ...
   * */
    ...
    * */
    public function action_edit()
    {
        ...
        $this->assign('position','修改景点'.$info['spotname']);
        $this->display('stourtravel/spot/edit');
    ...
    public function action_ajax_save()
    {
        ...
        $model->spotname = Arr::get($_POST,'spotname');
        $model->shortname = Arr::get($_POST,'shortname');
        $model->address = Arr::get($_POST,'address');
        $model->webid = $webid;
        $model->want = Arr::get($_POST,'want')?Arr::get($_POST,'want') : 0;  //想去
        $model->went = Arr::get($_POST,'went')?Arr::get($_POST,'went') : 0 ; //去过
 ...
 * */
    ...
    * */
    public function action_ticket_op()
    {
        ...
        $this->assign('spotname',$spotinfo['spotname']);
