...
class Controller_Photo  extends Stourweb_Controller{
	...
	public function action_photo()
	{
		...
		else if($action=='read')    //读取列表
		{
			...
			$w="a.id is not null";
			$w.=empty($keyword)?'':" and a.photoname like '%{$keyword}%'";
			...
			if(empty($kindid))
			{
			   $sql="select a.id,a.aid,a.photoname,a.litpic,a.headimgid,a.alt,a.title,a.kindlist,a.attrid,a.webid,a.photokind,a.themelist,if(length(ifnull(a.attrid,''))=0,0,1) as order_attrid,if(length(ifnull(a.kindlist,''))=0,0,1) as order_kindlist,if(length(ifnull(a.themelist,''))=0,0,1) as order_themelist,
a.addtime,a.modtime,a.ishidden,b.isjian,ifnull(b.displayorder,999999) as displayorder,b.isding from sline_photo as a left join sline_allorderlist b on (a.id=b.aid and b.typeid=6) where $w $order limit $start,$limit";
			...
			{
			   $sql="select a.id,a.aid,a.photoname,a.litpic,a.headimgid,a.alt,a.title,a.kindlist,a.attrid,a.webid,a.themelist,a.photokind,a.addtime,a.modtime,a.ishidden,b.isjian,if(length(ifnull(a.attrid,''))=0,0,1) as order_attrid,if(length(ifnull(a.kindlist,''))=0,0,1) as order_kindlist,if(length(ifnull(a.themelist,''))=0,0,1) as order_themelist,ifnull(b.displayorder,999999) as displayorder,b.isding from sline_photo as a left join sline_kindorderlist as b on (b.classid=$kindid and a.id=b.aid and b.typeid=6) where $w $order limit $start,$limit";
   ...
   */
    public function action_edit()
    {
        ...
        $this->assign('position','修改相册'.$info['photoname']);
        $this->display('stourtravel/photo/edit');
    }
    /*
     *
    ...
    public function action_ajax_photosave()
    {
        ...
        $data_arr=array();
        $data_arr['photoname']=Arr::get($_POST,'photoname');
        ...
        $data_arr['imginfo']=Arr::get($_POST,'imginfo');
