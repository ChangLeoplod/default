...
class Model_Line extends ORM {
	...
	public function deleteClear()
	{
		 ...
		 Common::deleteRelativeImage($this->linepic);
    ...
    {
        $sql="select a.id,a.aid,a.spotname,a.shortname,a.litpic,a.displayorder,a.webid from #@__spot as a where a.litpic !='' or a.piclist !='' group by a.spotname order by a.displayorder asc";


        $arr = ORM::factory('spot')->where("litpic !='' or piclist !=''")->group_by('spotname')->order_by('displayorder','ASC')->get_all();
    ...
    public function cloneLine($id, $num)
    {
        ...
        for($i = 1; $i <= $num; $i++)
        {
         ...
         //   $arr['linepic']=$this->clonePicture($arr['linepic'],$arr['webid']);
            $pics=self::clonePiclist($arr['linepic'],$arr['piclist']);

            $arr['webid'] = 0;
            $arr['linepic']=$pics['litpic'];
