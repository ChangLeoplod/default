...
define('UC_CLIENT_ROOT', SLINEROOT.'/uc_client');
...
class uc_note {
    ...
    function synlogout($get, $post)
    {
        ...
        $mobile = $get['username'];
        header('P3P: CP="CURa ADMa DEVa PSAo PSDo OUR BUS UNI PUR INT DEM STA PRE COM NAV OTC NOI DSP COR"');
        $result = $this->db->GetOne("SELECT mid,pwd FROM `#@__member` WHERE `mobile` = '$mobile' ");
        if(is_array($result))
        {
            include_once SLINEINC.'/member.class.php';
            //$User=new Member(7*3600);
            //$User->PutLoginInfo($result['mid']);
