...
if($action=='step5_getproductlist')
{
    ...
    if(!empty($destid))
    {
        ...
        $sql = "select a.id,a.aid,a.webid,a.linename,a.linepic as litpic,sellpoint from sline_line a left join #@__kindorderlist c on (c.classid=$destid and a.id=c.aid  and c.typeid=1) $where $orderby  limit 0,3";
        $arr = $dsql->getAll($sql);

    }
    else if(!empty($userplace))
    {
        $where=" where linename like '%$userplace%'";
        if(!empty($uday))$where.=" and lineday='$uday'";
        $orderby="order by case when c.displayorder is null then 9999 end,c.displayorder asc,a.modtime desc,a.addtime desc";
        $sql = "select a.id,a.aid,a.webid,a.linename,a.linepic as litpic,sellpoint from sline_line a left join #@__allorderlist c on (a.id=c.aid  and c.typeid=1) $where $orderby  limit 0,3";
        $arr = $dsql->getAll($sql);
    }
