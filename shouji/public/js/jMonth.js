...
(function($) {
	...
	jQuery.J.DrawCalendar = function(dateIn){
		...
		var prevMLink = jQuery('<div class="MonthNavPrev"><a href="javascript:;" class="link-prev">'+ defaults.navLinks.p +'</a></div>').click(function() {
		...
		var nextMLink = jQuery('<div class="MonthNavNext"><a href="javascript:;" class="link-next">'+ defaults.navLinks.n +'</a></div>').click(function() {
		...
		if(defaults.navLinks.enablePrevYear) {
			prevYLink = jQuery('<div class="YearNavPrev"><a href="javascript:;">'+ prevYear.getFullYear() +'</a></div>').click(function() {
		...
		if(defaults.navLinks.enableNextYear) {
			nextYLink = jQuery('<div class="YearNavNext"><a href="javascript:;">'+ nextYear.getFullYear() +'</a></div>').click(function() {
		...
		if(defaults.navLinks.enableToday) {
			...
			todayLink = jQuery('<div class="TodayLink"><a href="javascript:;" class="link-today">'+ defaults.navLinks.t +'</a></div>').click(function() {
		...
		monthNavHead.append(prevMLink);
		//if(defaults.navLinks.enableToday) { monthNavHead.append(todayLink); }

		monthNavHead.append(jQuery('<div class="MonthName"></div>').append(defaults.locale.months[d.getMonth()] + "月&nbsp;&nbsp;&nbsp;&nbsp;" + d.getFullYear()));
		...
		for (var i=defaults.firstDayOfWeek; i<defaults.firstDayOfWeek+7; i++) {
			...
			headRows=jQuery('<th title="' + wordday + '" class="DateHeader">' + wordday + '</th>');
			if(weekday == 0)
			{
				headRows.addClass('week');
			}
			if(weekday == 6)
			{
				headRows.addClass('noweek');
			}
			else
			{
				headRows.addClass('normal');
			}
			headRow.append(headRows);
		}
		...
		do {
			...
			for (var i=0; i<7; i++) {
				...
				//if (curDay >= 0 && curDay < maxDays )
				//{
				   var _cdate=_currentDate.getDate();	
				//}
				//else
				//{
				 //  var _cdate='';	
				//}
				 var dateLink = jQuery('<div class="DateLabel" onclick="hotelselectdate($(this).parent());">' + _cdate + '</div>').click(function(e) {
				...
				if(curDay < 0)
				{
					$myTd="<td onclick=\"hotelselectdate($(this));\"></td>";
				}
				else if (isCurrentMonth && curDay+1 == today.getDate()) 
				{
					$myTd="<td onclick=\"hotelselectdate($(this));\"></td>";
				}
				else
				{
					$myTd="<td onmouseover=\"this.className='mouseover'\" onmouseout=\"this.className='mouseout'\" onclick=\"hotelselectdate($(this));\"></td>";
				}
				var dateBox = jQuery($myTd).attr(atts).append(dateLink).dblclick(function(e) {
			...
			jQuery.each(calendarEvents, function(){
				...
				if(sDt) {
					if ((sDt >= _beginDate) && (sDt <= _endDate)) {			
						...
						var datades = jQuery('<a href="' + ev.URL + '">' + ev.Description + '</a>');
						datades.click(function(e) {
							defaults.onEventLinkClick(ev);
							e.stopPropagation();
						});
						
						var dataDescription = jQuery('<div class="Event" id="Event_' + ev.EventID + '"></div>').append(datades);
						cell.append(event);
						cell.append(dataDescription);
						event.fadeIn("normal");
					}
				}
			});
