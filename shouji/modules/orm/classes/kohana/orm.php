...
class Kohana_ORM extends Model implements serializable {
     ...
     * @return array
     */
    public function as_mutil_array()
    {
        $object = array();
     ...
     * return array
     * */
    public function get_sql($sql,$type=1)
    {
        //$type:1 selected,2 insert,3,update,4,delete
        //当$type=1时返回数组,2时返回数组array('插入id',影响行数),当为3或者4时返回影响行数.

       $result = DB::query($type,$sql)->execute();

       if($type == 1) //如果是selecte返回数组.
       {
           $out = array();
