...
<title>{if empty($row['seotitle'])}{$row['carname']}{else}{$row['seotitle']}{/if}-{$webname}</title>
...
<body>
  ...
  <div class="m-main">
  	...
  	<div class="xz-top-box">
    	...
    	<div class="txt">
        <p>{$row['carname']}</p>
        {if intval($list[0]['carprice'])>0}
        <span>&yen;{$list[0]['carprice']}起</span>
        {else}
        <span><电询</span>
        {/if}
      </div>
    ...
    <div class="big_box">
      ...
      <div class="price-tc">
        ...
        <ul>
        {loop $room $key $v1}
             <li {if $key==0}class="cur fangxing"{else}class="fangxing"{/if} dateid="{$v1['id']}">{$v1['suitname']} {if intval($v1['price'])>0}&yen;{$v1['price']}起{else}电询{/if}</li>
        {/loop}
        </ul>
      </div>
        ...
        {if !empty($room[0]['price_arr'])}
          <select style="font-size: 14px;width:100%;height:30px; color:#2a2a2a; margin-top:10px; margin-left:-5px;" id="tarveldate" name="tarveldate" onchange="settravel();">
            {loop $room[0]['price_arr'] $v2}
              <option value="{$v2['dayid']}||{$v2['adultprice']}">{$v2['day']} ￥{$v2['adultprice']}/{$room[0]['unit']}</option>
            {/loop}
          </select>
        {else}
      ...
      <input type="hidden" id="price" name="price" value="{$room[0]['price_arr'][0]['adultprice']}" />
      	<h3>预定人数</h3>
        <ul>
        	<li>
          	<label>预定数量：</label>
