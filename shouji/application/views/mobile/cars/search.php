                ...
                <div class="pdt_txt">
                  <div class="pdt_box">
                    <p class="p_tit">
                    	<em>{$v['carname']}</em>
                      <span>{$v['content']}</span>
                      <span class="mt5">优惠价：{if intval($v['carprice'])>0}<b>&yen;{$v['carprice']}</b>起{else}<b>电询</b>{/if}</span>
                    </p>
                  </div>
                </div>
              </a>
        ...
        var order = $('#order').val();
         $.get('/shouji/cars/list/kind/'+kind+'/attr/'+attrid+'/price/'+price+'/action/ajaxline/page/'+page+'/order/'+order,function(results){
            ...
            for(a in results){
                ...
                str += '<div class="pdt_list"><a href="{$cmsurl}cars/show/id/'+results[a]['id']+'"><div class="pdt_img"><img src="'+results[a]['litpic']+'" width="90" height="64"></div><div class="pdt_txt"><div class="pdt_box"><p class="p_tit"><em>'+results[a]['carname']+'</em><span>'+results[a]['content']+'</span><span class="mt5">优惠价：'+temprice+'</span></p></div></div></a></div>';
                listnum++;
