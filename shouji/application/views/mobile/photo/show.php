...
<title>{$info['photoname']}-{$webname}</title>
...
<body>
  {template 'public/top'}
    ...
    <a class="city_tit" href="javascript:;">{$info['photoname']}</a>
  </div>
  ...
  <div class="m-main">
    ...
    <div class="ph_box_con">
    	<h3><em>{$info['photoname']}</em><span>(<b style="font-weight: normal" id="cindex">1</b>/{count($picturelist)})</span></h3>
    	<p>{$info['imginfo']}</p>
    </div>
	</div>
