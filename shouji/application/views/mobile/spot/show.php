...
<body>
   {template 'public/top'}
    ...
    <a class="city_tit">{$row['spotname']}</a>
  </div>
  ...
  <div class="m-main">
    ...
    <div class="show-jc-js">
    	...
    	<div class="bt-box">
        <h1 class="tit">{$row['spotname']}</h1>
      </div>
      
      <ul>
      	<li class="li_1">
        	<p>
          	<span>{if $row['doorprice']>0}<em>&yen;{$row['doorprice']}</em> 起{else}<em>电询</em>{/if}</span>
          ...
          <div class="pdt_txt">
            <div class="pdt_box">
              <p class="p_tit">
                ...
                <span class="mt5">优惠价：{if $v['doorprice']>0}<b>&yen;{$v['doorprice']}</b> 起{else}<b>电询</b>{/if}</span>
              </p>
            </div>
          </div>
        </a>
