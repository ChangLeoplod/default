          ...
          {loop $list $vli}
              ...
              <a href="{$cmsurl}spot/show/id/{$vli['id']}">
                ...
                <div class="pdt_txt">
                  ...
                  <strong>电询</strong>
                  {else}
                  <strong>&yen;{$vli['doorprice']}</strong>
                  {/if}
        ...
        if(key!=''){
         ...
         $.get(url+'&starid='+starid+'&attrid='+attrid+'&pricetyle='+pricetyle+'&action=ajaxline&page='+page+'&order='+order,function(results){
            ...
            for(a in results){
                ...
                str += '<div class="pdt_list"><a href="{$cmsurl}spot/show/id/'+results[a]['id']+'"><div class="pdt_img"><img src="'+results[a]['litpic']+'" width="90" height="64"></div><div class="pdt_txt"><div class="pdt_box"><p class="p_tit"><em>'+results[a]['spotname']+'</em><span>'+results[a]['attrname']+'</span></p><p class="p_pir"><strong>'+temprice+'</strong><span>满意度'+results[a]['satisfyscore']+'</span></p></div></div></a></div>';
                listnum++;
