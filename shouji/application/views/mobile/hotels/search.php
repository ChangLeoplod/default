              ...
              <div class="pdt_txt">
                <div class="pdt_box">
                  <p class="p_tit">
                    ...
                    <span class="mt5">优惠价：{if intval($v['hotelprice'])>0}<b>&yen;{$v['hotelprice']}</b>起{else}<b>电询</b>{/if}</span>
                  </p>
                </div>
              </div>
            </a>
        ...
        var url='/shouji/hotels/list/?city='+city;
         $.get(url+'&star='+star+'&price='+price+'&key='+encodeURIComponent(key)+'&action=ajaxline&page='+page+'&order='+order,function(results){
            ...
            for(a in results){
               ...
               str +=  '<div class="pdt_list"><a href="{$cmsurl}hotels/show/id/'+results[a]['id']+'"><div class="pdt_img"><img src="'+results[a]['litpic']+'" width="90" height="64"></div><div class="pdt_txt"><div class="pdt_box"><p class="p_tit"><em>'+results[a]['hotelname']+'</em><span class="star">'+results[a]['randname']+'</span><span class="mt5">优惠价：'+temprice+'</span></p></div></div></a></div>';
