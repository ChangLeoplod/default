...
class Controller_Hotels extends Stourweb_Controller{
	...
	public function action_index(){
		...
		//定位第二级目的地
		 ...
		 //推荐酒店
		 $list=ORM::factory('hotel')->where("ishidden=0 and hotelprice>0")->order_by("displayorder",'asc')->limit('0,4')->get_all();
		 if(empty($list)){
		 	$list=ORM::factory('hotel')->where("ishidden=0")->order_by("displayorder",'asc')->limit('0,4')->get_all();
		 }
	...
	public function action_list()
	{
		...
		if(empty($city)){
			$w.=empty($keyword)?'':" and a.hotelname like '%{$keyword}%' ";
		}

		//如果有价格选择
 		if($price>0){
 			$pricearr =ORM::factory('hotel_pricelist')->where("id=$price")->find()->as_array();
 			$w.= " and a.hotelprice>=".intval($pricearr['min'])." and a.hotelprice<=".intval($pricearr['max']);
 		}
		//排序
		$orderby = empty($order)?'order by a.modtime desc':"order by a.hotelprice $order";
		...
		if(empty($city))
		{
		   $sql="select a.id,a.aid,a.hotelname,a.kindlist,a.attrid,a.hotelrankid,a.hotelprice,a.litpic,a.ishidden,ifnull(b.displayorder,9999) as displayorder from sline_hotel as a left join sline_allorderlist b on (a.id=b.aid and b.typeid=2) where $w $orderby limit $limit";
		}
		else
		{
			$sql="select a.id,a.aid,a.hotelname,a.kindlist,a.attrid,a.hotelrankid,a.hotelprice,a.litpic,a.ishidden,ifnull(b.displayorder,9999) as displayorder from sline_hotel as a left join sline_kindorderlist as b on (b.classid=$city and a.id=b.aid and b.typeid=2) where $w $orderby limit $limit";
		}
