...
class Controller_Raider extends Stourweb_Controller{
    ...
    public function action_index(){
        ...
        //带图片的文章
        $articlelist1 =  ORM::factory('article')->where("litpic is not NULL and litpic<>'' and ishidden=0")->order_by("displayorder",'asc')->limit(4)->get_all();
        //推荐文章
        $articlelist2 =  ORM::factory('article')->where("ishidden=0")->order_by("displayorder",'asc')->limit(3)->get_all();
        //去掉html标记
        ...
        if(!empty($data)){
            foreach ($data as $key => $value) {
                $temarr =  ORM::factory('article')->where("find_in_set(".$value['id'].",kindlist) and ishidden=0")->order_by("displayorder",'asc')->limit('0,3')->get_all();
                //去掉html标记
    ...
    public function action_list(){
        ...
        if(empty($kindid))
        {
          $sql="select a.id,a.litpic,a.allow,a.aid,a.articlename,a.attrid,a.kind,a.kindlist,a.litpic,a.content,a.isindex,a.webid,a.addtime,a.ishidden,a.themelist,a.iconlist,b.isjian,b.displayorder,b.isding,a.modtime from sline_article as a left join sline_allorderlist b on (a.id=b.aid and b.typeid=4) where $w $orderby limit $limit";
        }
        else
        {
           $sql="select a.id,a.litpic,a.allow,a.aid,a.articlename,a.attrid,a.kind,a.kindlist,a.litpic,a.content,a.isindex,a.webid,a.addtime,a.ishidden,a.themelist,a.iconlist,b.isjian,b.displayorder,b.isding,a.modtime from sline_article as a left join sline_kindorderlist as b on (b.classid=$kindid and a.id=b.aid and b.typeid=4)  where $w $orderby limit $limit";
    ...
    public function action_show(){
		...
		$row['content'] = preg_replace('/<img .*?src="(.*?)".*?>/is','<img src="$1"/>',$row['content']);
        //print_r($row);
        $this->assign('row',$row);
