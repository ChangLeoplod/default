...
class Controller_Cars extends Stourweb_Controller{
	...
	public function action_index(){
		 ...
		 $list = DB::select_array(array('id','carname','litpic','carprice'))->from('car')->where("ishidden=0 and carprice>0")->order_by("displayorder",'asc')->limit("0,4")->execute()->as_array();
		 if(empty($list)){
		 	$sql="SELECT a.id,a.carname,a.litpic,min(b.adultprice) as carprice FROM sline_car a LEFT JOIN sline_car_suit_price b ON (b.carid = a.id and b.adultprice!=0) WHERE a.ishidden=0 GROUP BY a.id ORDER BY a.displayorder ASC limit 0,4 ";
	...
	public function action_list()
	{
		...
		//排序
		$orderby = empty($order)?'modtime':"carprice";
    ...
    public function action_show(){
        ...
        $row['attrname']=Model_Car_Attr::getAttrnameList($v['attrid']);
