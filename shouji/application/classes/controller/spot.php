...
class Controller_Spot extends Stourweb_Controller{
		...
		//定位第一级目的地
		 ...
		 if(!empty($list_one)){
			 foreach($list_one as $k=>$v){
			 	...
			 	$sql="select 		a.aid,a.id,a.spotname,a.doorprice,a.area,a.spotpicid,a.tagword,a.kindlist,a.attrid,a.litpic,a.webid,a.piclist,a.themelist,a.iconlist,a.supplierlist,a.ishidden,b.isjian,ifnull(b.displayorder,9999) as displayorder,b.isding from sline_spot as a left join sline_kindorderlist as b on (b.classid={$v['id']} and a.id=b.aid and b.typeid=5) where $w $order limit 0,4";
	...
	public function action_list()
	{
		...
		if(empty($kindid)){
			$w.=empty($keyword)?'':" and a.spotname like '%{$keyword}%'";
		}
		...
		//如果有价格选择
 		if($pricetyle>0){
 			...
 			$w.= " and a.doorprice>=".intval($pricearr['min'])." and a.doorprice<=".intval($pricearr['max']);
 		}
		//排序
		$orderby = empty($order)?'order by a.modtime desc':"order by a.doorprice $order";
		...
		if(empty($kindid))
		{
		  $sql="select a.aid,a.id,a.spotname,a.doorprice,a.area,a.spotpicid,a.tagword,a.kindlist,a.attrid,a.litpic,a.webid,a.piclist,a.satisfyscore,a.themelist,a.iconlist,a.supplierlist,a.ishidden,b.isjian,ifnull(b.displayorder,9999) as displayorder,b.isding from sline_spot as a left join sline_allorderlist b on (a.id=b.aid and b.typeid=5) where $w $orderby  limit $limit ";
		}
		else
		{
	...
	public function action_show()
	{
		...
		if(empty($row['kindlist'])){
			...
			$sql="select a.id,a.spotname,a.doorprice,a.attrid,a.litpic,a.satisfyscore,ifnull(b.displayorder,9999) as displayorder from sline_spot as a left join sline_kindorderlist as b on (b.classid={$kindlistid} and a.id=b.aid and b.typeid=5) order by displayorder asc limit 0,4";
