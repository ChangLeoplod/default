...
class Controller_Mdd extends Stourweb_Controller{
    ...
    public function action_city(){
        ...
        //推荐数据调用
        //文章（有图片的跟没有图片的）
        $articlelist1 =  ORM::factory('article')->where("litpic is not NULL and litpic<>'' and find_in_set(".$kindid.",kindlist) and ishidden=1")->order_by("displayorder",'asc')->limit(4)->get_all();
        $articlelist2 =  ORM::factory('article')->where("find_in_set(".$kindid.",kindlist) and ishidden=1")->order_by("displayorder",'asc')->limit(3)->get_all();
        //去掉html标记
        ...
        //推荐线路，租车，酒店，门票,景区
        $lineslist = ORM::factory('line')->where("find_in_set(".$kindid.",kindlist) and ishidden=0")->order_by("displayorder",'asc')->limit(4)->get_all();
        $carlist = ORM::factory('car')->where("find_in_set(".$kindid.",kindlist) and ishidden=0")->order_by("displayorder",'asc')->limit(4)->get_all();
        $hotellist = ORM::factory('hotel')->where("find_in_set(".$kindid.",kindlist) and ishidden=0")->order_by("displayorder",'asc')->limit(4)->get_all();
        $spotlist = ORM::factory('spot')->where("find_in_set(".$kindid.",kindlist) and ishidden=0")->order_by("displayorder",'asc')->limit(4)->get_all();
        $photolist = ORM::factory('photo')->where("litpic is not NULL and litpic<>'' and find_in_set(".$kindid.",kindlist) and ishidden=1")->order_by("displayorder",'asc')->limit(4)->get_all();
