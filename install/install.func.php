...
function crView($conn)
{
		  ...
		  $sql2="create view sline_search (channelname,webid,aid,typeid,title,description,litpic,shownum,kindlist,attrid,headimgid) 
	  AS 
	  SELECT '线路',webid,aid,1,linename,features,linepic,shownum,kindlist,attrid,0 FROM sline_line
	  union
	  SELECT '酒店',webid,aid,2,hotelname,content,litpic,shownum,kindlist,attrid,0 FROM sline_hotel
	  union
	  SELECT '车辆',webid,aid,3,carname,content,litpic,shownum,kindlist,attrid,0 FROM sline_car
	  union
	  SELECT '攻略',webid,aid,4,articlename,content,litpic,shownum,kindlist,attrid,0 FROM sline_article
	  union
	  SELECT '景点',webid,aid,5,spotname,content,litpic,shownum,kindlist,attrid,0 FROM sline_spot
	  union
	  SELECT '相册',webid,aid,6,photoname,imginfo,litpic,shownum,kindlist,attrid,headimgid FROM sline_photo
	  union
	  SELECT '签证',webid,aid,8,title,content,litpic,shownum,'','',0 FROM sline_visa
	  union
	  SELECT '团购',webid,aid,13,title,content,litpic,shownum,kindlist,attrid,0 FROM sline_tuan";
