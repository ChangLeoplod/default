...
if(!empty($keyword))
{
	...
	$namewhere="a.spotname like '%{$keyword}%'";
...
//价格条件
    if(!empty($priceid)&& $priceid!=0)
	{
	   ...
	   $where.=" and a.doorprice >= {$pricearr['min']} and a.doorprice <= {$pricearr['max']} ";
	}
...
//排序条件
   ...
   $sql="select a.*,b.isjian ,b.isding,b.istejia from #@__spot a left join #@__kindorderlist b on (a.id=b.aid and b.typeid={$typeid} and a.webid=b.webid and b.classid='$dest_id') {$where} order by b.displayorder asc,a.modtime desc,a.addtime desc  ";
...
//针对名称搜索
   ...
   {
	    $where="where a.spotname like '%$searchkey%'";
		$wherecount="where a.spotname like '%$searchkey%'";
