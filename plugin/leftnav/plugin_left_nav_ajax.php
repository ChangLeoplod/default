...
if($dopost=='GetKindSon')
{
	  ...
	  $sql="select id,kindname,linkurl,color,displayorder,isopen from #@__plugin_leftnav where pid=$pid and webid=$sys_webid order by displayorder asc";
	  $dsql->SetQuery($sql);
      $dsql->Execute();
	  $treelevel=GetLevel($pid);
      $pleft=$padding*$treelevel;

	   while($row=$dsql->GetArray())
       {
		   $cid=$row['id'];
		     if($treelevel<2) //显示的级数
			   {
			      $clickstr="onClick='ShowKindSon(this,{$row['id']})'";
				  $imgicon='img/explode.png';
				  
			   }
			   else
			   {
			      $clickstr='';
				  $imgicon='img/contract.png';
		  ...
		  //  $clickstr="onClick='ShowKindSon(this,{$row['id']})'";
		   ...
		   $img= "<img style='cursor:pointer;padding-right:5px;' id='img{$row['id']}' {$clickstr} src='{$imgicon}' heightwidth=10 =10 />"; 
		   ...
		   $cell2="<div style='padding-left:{$pleft}px;'>{$img}<span class='areaname'>{$kindname}</span></div>";
		   $cell3="<span class='areaname'>{$kindurl}</span>";
		   $cell4=$color;
		   $cell5=GetSymbolHtml($row['id'],$row['isopen']);
		   $cell6="<span><a href='javascript:;' onclick=\"javascript:DeleteKind(this,{$row['id']})\">删除</a></span>";
	  ...
	  {
		$cell1= "&nbsp;";
		$cell2= "<div style='padding-left:{$pleft}px;'><div class='lastnode'> <input type='button' class='btn_sub'  onClick='javascript:addchild(this,{$pid})' value='自定义链接' onMouseOver=\"this.className='btn_sub_1'\" onMouseOut=\"this.className='btn_sub'\"></div></div>";
		$cell3= "&nbsp;";
		$cell5= "&nbsp;";
		$cell4= "&nbsp;";
		$cell6= "&nbsp;";
...
{
   $output=array();
   $sql="insert into #@__plugin_leftnav (kindname,pid,linkurl,color,webid) values('自定义','{$pid}','','','$sys_webid')";
   ...
   if( $dsql->ExecuteNoneQuery($sql))
   {
	  ...
	  if($treelevel<2) //显示的级数
			   {
			      $clickstr="onClick='ShowKindSon(this,{$id})'";
				  $imgicon='img/explode.png';
	  ...
	  $cell2="<div style='padding-left:{$pleft}px;'><img style='cursor:pointer;padding-right:5px;' id='img{$id}' {$clickstr} src='{$imgicon}' width=10 height=10 /><span class='areaname'>{$kindname}</span></div>";
      $cell3="<span class='areaname'>{$kindurl}</span>";
	  $cell4=$color;
	  $cell5=GetSymbolHtml($id,1);
      $cell6="<span><a href='javascript:;' onclick=\"javascript:DeleteKind(this,{$id})\">删除</a></span>";
		  
	   array_push($output,$cell1,$cell2,$cell3,$cell4,$cell5,$cell6,$id);
	  $json_string=json_encode($output);
	   print_r($json_string);
   }
...
if($dopost=='kind_saveorder') //保存排序
{
	  ...
	  $updatesql="update #@__plugin_leftnav set displayorder='{$displayorder}' where id='{$kindid}'";
...
if($dopost=='kind_savekind') 
{
	  ...
	  $updatesql="update #@__plugin_leftnav set kindname='{$kindname}' where id='{$kindid}'";
...
if($dopost=='kind_saveurl') 
{
	 if(!preg_match("/http:\/\//",$linkurl))
     {
	  ...
	  $updatesql="update #@__plugin_leftnav set linkurl='{$linkurl}' where id='{$kindid}'";
	  $dsql->ExecuteNoneQuery($updatesql);
...
if($dopost=='kind_savecolor') 
{
	  ...
	  $color=trim($color);
	  $updatesql="update #@__plugin_leftnav set color='{$color}' where id='{$kindid}'";
...
if($dopost=="DeleteKind")
{
   ...
   if(!childCheck($id))
   {
	   $sql="delete from #@__plugin_leftnav where id='$id'";
	   if(!$dsql->ExecuteNoneQuery($sql))
	   {
		   echo "false";
		   exit();
...
function GetLevel($id)
{
  ...
  $sql="select pid from #@__plugin_leftnav where id=$id";
  $row=$dsql->GetOne($sql);
  if(is_array($row) && $row['pid']!=0)
  {
     $level++;
	 GetLevel($row['pid']);
  }
...
//检查是否有子级
function childCheck($kindid)
{
	global $dsql;
    $flag=0;
	$sql="select count(*) as num from #@__plugin_leftnav where pid=$kindid";
...
if($dopost == 'tog_kind_open')
{
	...
	$updatesql="update #@__plugin_leftnav set isopen='$status' where id='$kindid'";
