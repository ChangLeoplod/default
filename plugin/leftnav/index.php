...
<script language="javascript" src="plugin_left_nav.js" type="text/javascript"></script>
...
<div id="breadnav">
 ...
 <span class="onlink">首页左侧滑动导航设置</span>   
</div>
...
function GetKindList($webid)
{
	  global $dsql,$sys_webid;
	     ...
	     echo " <td width='15%' height='27' align='left' class='header' >导航名</td>";
		
		 echo "<td width='25%' align='left' class='header'>链接地址</td>";
		 echo "<td width='8%' align='left' class='header'>文字颜色</td>";
	     ...
	     echo "</tr>";
	       ...
	       $sql="select id,kindname,linkurl,color,displayorder,isopen from #@__plugin_leftnav where pid=0 and webid=$webid order by displayorder asc";
		  ...
		  while($row = $dsql->GetArray())
         {
		   ...
		   echo "<div>{$img}<span class='areaname'>{$kindname}</span></div></td>";
			 ...
			 echo "<td width='10%' align='left' id='td{$cid}'><span><a href='javascript:;' onclick=\"javascript:DeleteKind(this,{$row['id']})\">删除</a></span></td>";
             echo "</tr>";
		    
			}	
			
			echo "<tr class='tdbg'><td class='order'>&nbsp;</td><td colspan='5'><div style=''><div class='lastnode'> <input type='button' class='btn_sub'  onClick='javascript:addchild(this,0)' value='自定义链接' onMouseOver=\"this.className='btn_sub_1'\" onMouseOut=\"this.className='btn_sub'\"></div></div></td></tr>";	 
		    //echo "</td>";
            echo "</table>";
...
oPopBody.style.fontSize = "12px"; 
   ...
   var currentcolor=e.value;
    ...
    ocbody += "<tr height=\"20\" width=\"20\"><td align=\"center\"><table style=\"border:1px solid #808080;\" width=\"12\" height=\"12\" bgcolor=\""+currentcolor+"\"><tr><td></td></tr></table></td><td bgcolor=\"eeeeee\" colspan=\"7\" style=\"font-size:12px;\" align=\"center\">当前颜色&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"javasrcipt:;\" onclick=\"hidePopup()\">关闭</a></td></tr>";
