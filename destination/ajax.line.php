...
if($dopost == 'getlinelist')
{
	 ...
	 //按名称搜索
	 if(!empty($searchkey)||($searchkey!=0))$where.=" and linename like '%{$searchkey}%'";
	 ...
	 if(!empty($attrlist))
	 {
	  ...
	  foreach($arr as $row)
	  {
            ...
            $lineprice=!empty($real) ? $real : $row['lineprice'];//线路报价
		    ...
		    $out.='<div class="list_con">
          	<div class="list_pic">
            	<a class="fl" href="'.$url.'"><img class="fl" src="'.$litpic.'" width="150" height="115" title="'.$row['linename'].'" alt="'.$row['linename'].'" /></a>
            </div>
           	<div class="list_txt">
            	<p class="p1"><a href="'.$url.'" target="_blank">'.$row['linename'].'</a></p>
