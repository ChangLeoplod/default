...
class SlineSql
{
    ...
    function SlineSql($pconnect=FALSE,$nconnect=TRUE)
    {
    ...
    //用指定参数初始数据库信息
    function SetSource($host,$username,$pwd,$dbname,$dbprefix="sline_")
    {
        $this->dbHost = $host;
    ...
    function Open($pconnect=FALSE)
    {
        ...
        if(!$this->linkID)
        {
            $this->DisplayError("思途CMS错误警告：<font color='red'>连接数据库失败，可能数据库密码不对或数据库服务器出错！</font>");
            exit();
        }
    ...
    function SetQuery($sql)
    {
		...
		if($this->safeCheck) 
		{
		   CheckSql($this->queryString);//检查安全性
		}
    ...
    function DisplayError($msg)
    {
        ...
        $emsg = '';
        $emsg .= "<div><h3>STcms Error Warning!</h3>\r\n";
        $emsg .= "<div><a href='http://www.stourweb.com' target='_blank' style='color:red'>Technical Support: http://www.stourweb.com</a></div>";
		...
		$fsize=filesize($errorTrackFile);
       if($fsize>=5*1024*1024)
		{
		    $fp = @fopen($errorTrackFile, 'w');
...
if (!function_exists('CheckSql'))
{
    function CheckSql($db_string,$querytype='select')
    {
        ...
        $old_pos = 0;
        $pos = -1;
        $log_file = SLINEINC.'/../data/'.md5($cfg_cookie_encode).'_safe.txt';
        $userIP = GetIP();
        $getUrl = GetCurUrl();
