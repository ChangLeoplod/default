 ...
 * 模板引擎文件
 *
 * @version        $Id: sttemplate.class.php  netman $
...
function MakePublicTag($atts=array(),$refObj='',$fields=array())
{
    ...
    $plusfile = SLINEINC.'/tpllib/plus_'.$atts['tagname'].'.php';
...
//自定义钩子
function MakeHookTag($atts=array(),$refObj='',$fields=array())
{
    $atts['hook'] = preg_replace("/[0-9]{1,}$/", "", $atts['hook']);
	$func=$atts['bind'];
    $file = SLINEADMIN.'/inc/hook/'.$atts['hook'].'.php';
    if(!file_exists($file))
    {
        if(isset($atts['rstype']) && $atts['rstype']=='string')
        {
...
class STTemplate
{
    ...
    var $refObj = '';
    var $makeLoop = 0;
    var $tagStartWord =  '{sline:';
    var $fullTagEndWord =  '{/sline:';
    var $sTagEndWord = '/}';
    var $tagEndWord = '}';
    var $tpCfgs = array();
    ...
    function __construct($templatedir='',$refDir='')
    {
        ...
        if($templatedir=='')
        {
            $this->templateDir = SLINEROOT.'/templates/cache/';
        }
        else
        {
        ...
        $this->cacheDir = SLINEROOT.$GLOBALS['cfg_tplcache_dir'];
		
    }

    //构造函数,兼容PHP4
    function STTemplate($templatedir='',$refDir='')
    {
     ...
     * @return    void
     */
    function SetTagStyle($ts='{sline:',$ftend='{/sline:',$stend='/}',$tend='}')
    {
    ...
    function CompilerOneTag(&$cTag)
    {
        ...
        else if($tagname == 'include')
        {
			...
			if($cTag->GetAtt('if')!='') //添加条件判断
			{
			  if(eval("return {$cTag->GetAtt('if')} ;" )) 
			  {
		...
		else if( $tagname=='taghook')
		{
			...
			$rsvalue = '<'.'?php'."\r\n".'$atts = array();'."\r\n".$rsvalue;
		    $rsvalue .= '$blockValue = MakeHookTag($atts,$this->refObj,$fields); '."\r\n";
