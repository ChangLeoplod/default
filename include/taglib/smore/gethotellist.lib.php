...
function lib_gethotellist(&$ctag,&$refObj)
{
    ...
    $revalue = '';
	$basefield = 'a.id,a.sellpoint,a.aid,a.kindlist,a.hotelname,a.address,a.litpic,a.ishidden,a.hotelrankid,a.areaid,a.webid,a.hotelrankid as hotelrankids,a.areaid as areaids,a.shownum,a.iconlist';
	...
	else if($type=='theme')
	{
		...
		$sql="select a.*,b.hotelrank,a.hotelprice as price from #@__hotel a left join #@__hotel_rank b on (a.hotelrankid=b.aid and a.webid=b.webid)  where a.ishidden='0' and FIND_IN_SET($themeid,a.themelist)  order by a.modtime desc,a.addtime desc,a.shownum desc limit {$limit},{$row}";
	...
	else if($type=='top')
	{
		...
		if($flag=='recommend')
		{
		   $sql="select {$basefield},b.displayorder,b.isding,b.isjian,b.istejia,a.ishidden,a.hotelprice from #@__hotel a left join #@__allorderlist b on(a.id=b.aid and b.typeid=2)  where a.ishidden='0' order by case when b.displayorder is null then 9999 end,b.displayorder asc,a.modtime desc,a.addtime desc  limit {$limit},{$row}";
		...
		else if($flag=='hot')
		{
		   ...
		   $sql="select a.*,b.hotelrank,a.hotelprice as price from #@__hotel a left join #@__hotel_rank b on (a.hotelrankid=b.aid and a.webid=b.webid)  where a.ishidden='0'   group by a.hotelname order by a.shownum desc,a.modtime desc,a.addtime desc limit {$limit},{$row}";
