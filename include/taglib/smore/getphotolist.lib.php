...
function lib_getphotolist(&$ctag,&$refObj)
{
	...
	$basefield = 'a.aid,a.photoname,a.seotitle,a.litpic,a.imginfo,a.title as littitle,a.alt,a.author,a.shownum,a.webid,a.headimgid';
    $revalue = '';
   //加目的地页面显示条件
	...
	{  
	   if(isset($refObj->Fields['sonid']))
	   {
			...
			$sql="select aid,photoname,seoname,headimg,imginfo,title as littitle,alt,author,shownum,webid,headimgid from #@__photo  where webid!='' {$destwhere} and   photokind={$sonid} order by modtime desc,addtime desc limit {$limit},{$row}";
