...
{
    global $dsql,$id,$attr_id;
	$attlist="row|10,flag|,id|0";
	$webid=0;
	...
	{
	   $sql="select id,attrname,attrid from #@__line_attr where  CONCAT(attrid,',') not like '%162,%' and webid=0 and isopen='1' and pid!=0 order by displayorder asc limit 0,{$row}";
	}
	   ...
	   else if($flag=='attr')
	   {
		   ...
		   if(CheckExistAttrGuide('1',$asonid,$row['id']))
		   {
			   ...
			   $row['title']=$row['attrname'];
                           if ($attr_id == $row['attr_id']) {
                               $row['class'] = ' class="hover"';
                           }
...
function CheckLinePrice($min,$max)
{
	...
	if($kindid != 0)
	{
		$sql="select 1 from #@__line where lineprice>=$min and lineprice<=$max  and ishidden=0 and FIND_IN_SET($kindid,kindlist) limit 1";
	}
	else if($destid != 0)
	{
		$sql="select 1 from #@__line where lineprice>=$min and lineprice<=$max  and ishidden=0 and FIND_IN_SET($destid,kindlist) limit 1";
	}
	else
	{
		$sql="select 1 from #@__line where lineprice>=$min and lineprice<=$max and ishidden=0 limit 1";
	}
