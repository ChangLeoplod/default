...
function lib_helplist(&$ctag,&$refObj)
{
	...
	if($flag == 'all')
	{
		$sql="select id,kindname,imgsrc from #@__help_kind where webid=0 and isopen=1 order by displayorder asc";
	}
	else
	{
		if($helpkindid=='')
	    {
		  $sql="select id,kindname,imgsrc from #@__help_kind where webid=0 and isopen=1 order by displayorder asc limit 0,{$row}";
		}
		else
		{
		  $sql="select id,kindname,imgsrc from #@__help_kind where webid=0 and isopen=1 and id=$helpkindid order by displayorder asc limit 0,{$row}";
		}
