...
function lib_channel(&$ctag,&$refObj)
{
    ...
    else if($type=='son')
    {
		...
		$pid=$row['id'];
		if($typeid==0){
        $sql = "SELECT aid,shortname,linktype,url From `#@__nav` WHERE pid='$pid' And isopen=1 and webid=$sys_webid and isopen=1 order by displayorder asc limit 0, $line ";}
		else{$sql = "SELECT aid,shortname,linktype,url From `#@__nav` WHERE pid='$pid' and webid=$sys_webid order by displayorder asc limit 0, $line ";}
    }
	else if($type=='special') //首页左侧出行帮助
	{
		...
		$row=$dsql->GetOne($getsql);
		$pid=$row['id'];
		$sql = "SELECT aid,shortname,linktype,url From `#@__nav` WHERE pid='$pid'  and webid=$sys_webid and isopen=1 order by displayorder asc limit 0, $line ";
	}
