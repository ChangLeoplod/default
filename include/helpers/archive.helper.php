...
//获取attrid的选中状态，如果选中，则返回参数1，也就是$class
  ...
  { 
	   ...
	   if(is_array($arr))
       {
           ...
           if($flag)
           {
               ...
               if($arr['paytype']=='3') //二次确认支付
               {
                   ...
                   $emailInfo=self::getEmailMsgConfig2($arr['typeid'],1);
    ...
    * */
    ...
    {
        self::loadModule('common');
        $model = new CommonModule('#@__email_msg');
        $row = $model->getOne("msgtype='{$msgtype}'");
        return $row;
    }
...
$paytypeArr=explode(',',$GLOBALS['cfg_pay_type']);
        if($paytype!=4)
        {
            ...
            if(in_array(13,$paytypeArr)&&in_array(1,$paytypeArr))
            {
				...
				<input type="radio" class="raw" name="choosepay" checked="checked"  value="13" data-type="支付宝"/>
