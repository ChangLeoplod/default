...
function getRecSpots_default($lineid,$webid)
{
	...
	$str4 = "";
	$temp = "";
	$sql="select spotname,litpic,spotid from #@__line_spot where lineid='$lineid' and webid=0 order by displayorder asc";
	$res = $dsql->getAll($sql);
	$numberrow=count($res);
	...
	foreach($res AS $row)
	{
		if($idx < 5)
		{
			...
			$str3.="<div class='ty'><a href='{$weburl}/spots/show_{$row['spotid']}.html' target='_blank'><img src='{$weburl}{$row['litpic']}' border='0' width='100' height='75'></a>"."<a href='{$weburl}/spots/show_{$row['spotid']}.html' class='spotname' target='_blank'>{$row['spotname']}</a></div>";
		}
		else
		{
			$str4 .= "<li><a href=\"{$weburl}/spots/show_{$row['spotid']}.html\">{$row['spotname']}</a></li>";
		}
		$idx++;
	}
	$str5 = "</div>";
...
function getRecSpots_standard($lineid,$webid)
{
	...
	foreach($res AS $row)
	{
			...
			$str.="<li><a class=\"imgGroup\" href=\"{$weburl}/spots/show_{$row['spotid']}.html\"><img width=\"125\" height=\"135\"  src=\"{$row['litpic']}\"></a><p>{$row['spotname']}</p></li>";
...
function getRecSpots_xizang($lineid,$webid)
{
	...
	foreach($res AS $row)
	{
		if($idx > 8)
		{
			...
			$str3middle.="<li><span><a href=\"/spots/show_{$row['spotid']}.html\" target=\"_blank\"><img src=\"{$row['litpic']}\" alt=\"{$row['spotname']}\"/></a></span><span><a href=\"/spots/show_{$row['spotid']}.html\" target=\"_blank\">{$row['spotname']}</a></span></li>";
		
		}
		else
		{
			$str2middle.=" <dd><a href=\"/spots/show_{$row['spotid']}.html\">{$row['spotname']}</a></dd>";
		}
		$idx++;
	}
    ...
    else
    {
        ...
        elseif($row['isstyle'] == 2) //按天数上传的行程
        {
            ...
            for($i=1; $i<=$row['lineday']; $i++)
            {
                ...
                //酒店信息
                $hotelinfo = getInfo('#@__hotel',"where hotelname='$detail[3]'");
                ...
                $out.=' <div class="dinner_top "'.$repast_style.'>
                  ...
                  <div class="hotel_top" '.$hotelstyle.'>
                    <div class="hotel_bg_pic"></div>
                    <h3>'.$hotelinfo['hotelname'].'<span>（因季节不同，酒店需求不同，酒店只为参考，入住以实际安排为主）</span></h3>
                  </div>
...
function getHotelPic($hotelinfo,$type='big')
{
	...
	foreach($picarr as $pic)
	{
	     ...
	     $p = explode('||',$pic);
		 $picname = !isset($p[1]) ? $p[1] : $hotelinfo['hotelname'];//图片名称
