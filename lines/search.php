...
if(!empty($keyword))
{
	...
	$namewhere="a.linename like '%{$keyword}%'";
...
{ 
  ...
  $fields="a.id,a.iconlist,a.aid,a.webid,a.lineicon,a.linename as title,a.piclist,a.seotitle,a.sellpoint,a.linepic as litpic,a.storeprice,a.lineprice,a.linedate,a.transport,a.lineday,a.startcity,a.overcity,a.shownum,a.satisfyscore,a.bookcount,a.features,a.jifenbook,a.jifentprice,a.jifencomment,a.paytype,a.attrid,a.storeprice";
...
if(!empty($dest_id))
{
    $sql="select {$fields},a.linepic as litpic,b.isjian,b.isding,b.istejia from #@__line as a left join #@__kindorderlist b on(a.id=b.aid and b.typeid=1 and b.classid=$dest_id ) {$where}{$orderby},modtime desc,addtime desc ";
}
else
{
    $sql="select {$fields},a.linepic as litpic,b.isjian,b.isding,b.istejia from #@__line as a left join #@__allorderlist b on(a.id=b.aid and b.typeid=1 ) {$where}{$orderby},modtime desc,addtime desc ";
}
...
function getLinesNum($childid,$attrid,$priceid,$day)
{
	...
	if($priceid != 0)
	{
		...
		$where .= " and lineprice > '$parr[min]' and lineprice < '$parr[max]'";
	}
