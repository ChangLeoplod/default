...
if($dopost == 'getLineByDestId')
{
	...
	foreach($arr as $row)
	{
		...
		$litpic = getUploadFileUrl($row['linepic']);
...
if($dopost == 'getdayspots')
{
    ...
    {
        $spotinfo = getInfo('#@__spot',"where id='{$row['spotid']}' ",'id,aid,content,litpic,spotname,lng,lat');
        ...
        $outstr .= "<h4 style='margin:0 0 5px 0;padding:0.2em 0'><a href='".$linkurl."' target='_blank'>".$spotinfo['spotname']."</a></h4>";
        $outstr .= "<img style='float:right;margin:4px' id='imgDemo' src='".$spotpic."' width='139' height='104' title='".$spotinfo['spotname']."'/>"; 
